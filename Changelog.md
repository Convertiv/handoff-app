...
Under some conditions, this would fail depending on your node configuration.
...
* When developing in handoff, changes to components cause the Nextjs HMR to fail
...
A minor bugfix release that fixes some issues when running handoff-app as a
CLI tool.
...
* Components no longer use semver versioning in the component structure. You will
  ...
  * Instead of {component\_id}/1.0.0/{data}, the structure should be {component\_id}/{data}
...
Here's a simple example that will fetch the data down. This example expects a DEV\_ACCESS\_TOKEN and a FIGMA\_PROJECT\_ID env variable, or those to be provided in `process.env`. If not supplied, they will be prompted for when the pipeline is run.
