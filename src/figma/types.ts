...
export interface VectorBase extends Global {
   ...
   */
  readonly fillOverrideTable: Record<number, PaintOverride>;
   ...
   */
  readonly strokeCap?: 'NONE' | 'ROUND' | 'SQUARE' | 'LINE_ARROW' | 'TRIANGLE_ARROW';
  /**
  ...
  /**
   * Only valid if strokeJoin is "MITER". The corner angle, in degrees, below which strokeJoin will be set to "BEVEL" to avoid super sharp corners.
...
/** A text box */
export interface Text extends Omit<VectorBase, 'fillOverrideTable'> {
   ...
   * List types are represented as string enums with one of these possible values:
   *
   ...
   */
  readonly lineTypes?: ReadonlyArray<'NONE' | 'UNORDERED' | 'ORDERED'>;
  /**
   * An array with the same number of elements as lines in the text node, where lines are delimited by newline or paragraph separator characters.
   * Each element in the array corresponds to the indentation level of a specific line.
   */
  readonly lineIndentations?: ReadonlyArray<number>;
}
...
export interface Component extends FrameBase {
  ...
  readonly componentPropertyDefinitions: Record<string, ComponentPropertyDefinition>;
}
...
export interface TypeStyle {
   ...
   * @default "ORIGINAL"
   */
  readonly textCase?: 'UPPER' | 'LOWER' | 'TITLE' | 'SMALL_CAPS' | 'SMALL_CAPS_FORCED' | 'ORIGINAL';
  /**
   * Text decoration applied to the node, default is none

   * @default "NONE"
   */
  readonly textDecoration?: 'STRIKETHROUGH' | 'UNDERLINE' | 'NONE';
...
export interface FileResponse {
  ...
  readonly componentSets: {
    readonly [key: string]: ComponentSet;
  };
