...
export const DisplayLogo: React.FC<{ logo: DisplayableAsset; content?: string; correct?: boolean }> = ({ logo, content }) => {
  // Workspace/static ships the SVG body inline in tokens.json (`data`) — render it directly so the
  // logo has no dependency on the fetch-only `/api/assets/logos/*` files (absent on a static host).
  // Registry has no inline body; it falls back to the individual asset content URL (`src`).
