...
## Config: including pages

In `handoff.config.js`, add an `entries.pages` array of paths to scan for page definitions (same pattern as `entries.components`):
...
- **Consumption**: The static Next app reads `pages.json` for the page list (e.g. `fetchPages()`) and loads `api/page/{id}.json` and the HTML preview URLs for the page detail view. See **[rest_api.md](./rest_api.md)** and **[rest.yml](./rest.yml)** for the full API overview and OpenAPI spec.
