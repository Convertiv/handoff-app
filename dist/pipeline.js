...
import generateChangelogRecord from './changelog.js';
...
var tokensFilePath = function (handoff) { return path.join(outputPath(handoff), 'tokens.json'); };
var previewFilePath = function (handoff) { return path.join(outputPath(handoff), 'preview.json'); };
var changelogFilePath = function (handoff) { return path.join(outputPath(handoff), 'changelog.json'); };
var variablesFilePath = function (handoff) { return path.join(outputPath(handoff), 'tokens'); };
var iconsZipFilePath = function (handoff) { return path.join(outputPath(handoff), 'icons.zip'); };
var logosZipFilePath = function (handoff) { return path.join(outputPath(handoff), 'logos.zip'); };
/**
...
var buildCustomFonts = function (handoff, documentationObject) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, fontTransformer(handoff, documentationObject)];
            case 1: return [2 /*return*/, _a.sent()];
        }
    });
}); };
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                return [4 /*yield*/, integrationTransformer(handoff, documentationObject)];
            case 1:
                _a.sent();
                _a.label = 2;
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, Promise.all([
                    previewTransformer(handoff, documentationObject).then(function (out) { return fs.writeJSON(previewFilePath(handoff), out, { spaces: 2 }); }),
                ])];
            case 1:
                _a.sent();
                ...
                return [4 /*yield*/, buildClientFiles(handoff)
                        .then(function (value) { return !!value && console.log(chalk.green(value)); })
 ...
 * @param documentationObject
*/
var buildStorybookPreview = function (handoff, documentationObject) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, Promise.all([
...
var buildStyles = function (handoff, documentationObject) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                typeFiles = scssTypesTransformer(documentationObject);
                typeFiles = handoff.hooks.typeTransformer(documentationObject, typeFiles);
                cssFiles = cssTransformer(documentationObject);
                cssFiles = handoff.hooks.cssTransformer(documentationObject, cssFiles);
                scssFiles = scssTransformer(documentationObject);
                scssFiles = handoff.hooks.scssTransformer(documentationObject, scssFiles);
                sdFiles = sdTransformer(documentationObject);
                sdFiles = handoff.hooks.styleDictionaryTransformer(documentationObject, sdFiles);
                mapFiles = mapTransformer(documentationObject);
                mapFiles = handoff.hooks.mapTransformer(documentationObject, mapFiles);
                return [4 /*yield*/, Promise.all([
                        fs
                            .ensureDir(variablesFilePath(handoff))
                            .then(function () { return fs.ensureDir("".concat(variablesFilePath(handoff), "/types")); })
                            .then(function () { return fs.ensureDir("".concat(variablesFilePath(handoff), "/css")); })
                            .then(function () { return fs.ensureDir("".concat(variablesFilePath(handoff), "/sass")); })
                            .then(function () { return fs.ensureDir("".concat(variablesFilePath(handoff), "/sd/tokens")); })
                            .then(function () { return fs.ensureDir("".concat(variablesFilePath(handoff), "/maps")); })
                            .then(function () { return Promise.all(Object.entries(sdFiles.components).map(function (_a) {
                            var name = _a[0], _ = _a[1];
                            return fs.ensureDir("".concat(variablesFilePath(handoff), "/sd/tokens/").concat(name));
                        })); })
                            .then(function () {
                            return Promise.all(Object.entries(typeFiles.components).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/types/").concat(name, ".scss"), content);
                            }));
                        })
                            .then(function () {
                            ...
                            return Promise.all(Object.entries(cssFiles.components).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/css/").concat(name, ".css"), content);
                            }));
                        })
                            .then(function () {
                            ...
                            return Promise.all(Object.entries(scssFiles.components).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/sass/").concat(name, ".scss"), content);
                            }));
                        })
                            .then(function () {
                            ...
                            return Promise.all(Object.entries(sdFiles.components).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/sd/tokens/").concat(name, "/").concat(name, ".tokens.json"), content);
                            }));
                        })
                            .then(function () {
                            return Promise.all(Object.entries(sdFiles.design).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/sd/tokens/").concat(name, ".tokens.json"), content);
                            }));
                        })
                            .then(function () {
                            return Promise.all(Object.entries(mapFiles.components).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(variablesFilePath(handoff), "/maps/").concat(name, ".json"), content);
                            }));
                        })
                            .then(function () {
                            ...
                            return Promise.all(Object.entries(mapFiles.attachments).map(function (_a) {
                                var name = _a[0], content = _a[1];
                                return fs.writeFile("".concat(outputPath(handoff), "/").concat(name, ".json"), content);
                            }));
                        }),
                    ])];
            case 1:
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                missingEnvVars = true;
                console.log(chalk.yellow("Figma developer access token not found. You can supply it as an environment variable or .env file at HANDOFF_DEV_ACCESS_TOKEN.\nUse these instructions to generate them ".concat(chalk.blue("https://help.figma.com/hc/en-us/articles/8085703771159-Manage-personal-access-tokens"), "\n")));
            ...
            case 5:
                ...
                return [3 /*break*/, 14];
            case 6:
                envFilePath = path.resolve(handoff.workingPath, '.env');
            ...
            case 8:
                ...
                return [4 /*yield*/, fs.appendFile(envFilePath, envFileContent)];
            case 9:
                _a.sent();
                ...
                return [3 /*break*/, 12];
            case 10: return [4 /*yield*/, fs.writeFile(envFilePath, envFileContent.replace(/^\s*[\r\n]/gm, ""))];
            case 11:
                _a.sent();
            ...
            case 13:
                error_1 = _a.sent();
                console.error(chalk.red('Error handling the .env file:', error_1));
                return [3 /*break*/, 14];
            case 14:
...
var figmaExtract = function (handoff) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 2:
                changelog = (_b.sent()) || [];
                return [4 /*yield*/, fs.emptyDir(outputPath(handoff))];
            case 3:
                _b.sent();
            ...
            case 6:
                ...
                return [4 /*yield*/, createDocumentationObject(handoff.config.figma_project_id, handoff.config.dev_access_token, legacyDefinitions)];
            case 7:
                ...
                return [4 /*yield*/, Promise.all(__spreadArray([
                        ...
                        ? [
                            zipAssets(documentationObject.assets.icons, fs.createWriteStream(iconsZipFilePath(handoff))).then(function (writeStream) {
                                return stream.promises.finished(writeStream);
                            }),
                            zipAssets(documentationObject.assets.logos, fs.createWriteStream(logosZipFilePath(handoff))).then(function (writeStream) {
                                return stream.promises.finished(writeStream);
                            }),
                        ]
                        : []), true))];
            case 8:
                ...
                if (!!fs.existsSync(outputFolder)) return [3 /*break*/, 10];
            ...
            case 10:
                ...
                fs.copyFileSync(iconsZipFilePath(handoff), path.join(handoff.modulePath, '.handoff', "".concat(handoff.config.figma_project_id), 'public', 'icons.zip'));
                fs.copyFileSync(logosZipFilePath(handoff), path.join(handoff.modulePath, '.handoff', "".concat(handoff.config.figma_project_id), 'public', 'logos.zip'));
 ...
 * @param handoff
 */
export var buildIntegrationOnly = function (handoff) { return __awaiter(void 0, void 0, void 0, function () {
 ...
 * @param handoff
 */
export var buildPreviewOnly = function (handoff) { return __awaiter(void 0, void 0, void 0, function () {
    var documentationObject;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, readPrevJSONFile(tokensFilePath(handoff))];
            case 1:
                documentationObject = _a.sent();
                if (!documentationObject) return [3 /*break*/, 3];
                return [4 /*yield*/, buildStorybookPreview(handoff, documentationObject)];
            case 2:
                _a.sent();
                _a.label = 3;
            case 3: return [2 /*return*/];
    ...
    return __generator(this, function (_b) {
        try {
            ...
            exportables = definitions
                .map(function (def) {
                var _a;
                var defPath = path.resolve(path.join(handoff.modulePath, 'config/exportables', "".concat(def, ".json")));
                var projectPath = path.resolve(path.join(handoff.workingPath, 'exportables', "".concat(def, ".json")));
